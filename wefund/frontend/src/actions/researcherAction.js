...
import { multipartTokenConfig } from "./config";
...
export const registerResearcher =
  ({ id_card_number, id_card_copy, date_of_birth, degree, organisation, cv }) =>
  (dispatch, getState) => {
    // Is Loading
    ...
    axios
      .post(
        "/api/account/researcher/",
        formData,
        multipartTokenConfig(getState)
      )
      .then((res) => {
        dispatch({ type: "RESEARCHER_ADD_SUCCESS", payload: res.data });
        dispatch(loadUser());
      })
      .catch((err) => {
        dispatch({ type: "RESEARCHER_ADD_FAIL", payload: err.response.data });
      });
  };
...
export const updateResearcher =
  ({ id_card_number, id_card_copy, organisation, date_of_birth, degree, cv }) =>
  (dispatch, getState) => {
    dispatch({ type: "RESEARCHER_LOADING" });
